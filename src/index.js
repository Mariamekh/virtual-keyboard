...
class Keyboard {
  ...
  typeIn(event) {
    event.preventDefault();
    const lookedKey = keyArr.find((k) => k.code === event.code);
    if (!lookedKey) return;
    let key = lookedKey.text[this.lang];
    key = this.shift ? lookedKey.upKey[this.lang] : lookedKey.text[this.lang];
  ...
  capsKey() {
    this.keys.forEach((btn) => {
      let key = keyArr.find((k) => k.text[this.lang] === btn.innerHTML || k.upKey[this.lang] === btn.innerHTML);
      let changeKey = false;
      if (key) {
  ...
  switchLang() {
    ...
    for (const btn of this.keys) {
      let key = keyArr.find((k) => k.text[notLang] === btn.innerHTML || k.upKey[notLang] === btn.innerHTML);
      if (key) {
...
window.onload = () => {
  const renderedKeyboard = new Keyboard(localStorage.getItem("lang") || "en");
  renderedKeyboard.renderKeyboard();
};
